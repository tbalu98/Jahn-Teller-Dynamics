...
def _read_config(cfg_path: Path) -> dict:
    ...
    results_folder: base for npz and out. Paths relative to repo root.
    """
    cp = ConfigParser()
    cp.read(cfg_path)
    ...
    results_folder = repo_root
    if "data_folder" in opts:
        data_folder = resolve(opts["data_folder"], repo_root)
    if "results_folder" in opts:
        results_folder = resolve(opts["results_folder"], repo_root)

    result = {"results_folder": str(results_folder)}
    if "npz" in opts:
        result["npz"] = str(resolve(opts["npz"], results_folder))
    if "dipole" in opts:
    ...
    if "out" in opts:
        # Keep as path relative to results_folder (e.g. subdir/spectrum.png)
...
def main() -> int:
    ...
    parser.add_argument("--results-folder", type=str, default="", help="Base folder for all output files")
    parser.add_argument("--out", type=str, default="", help="Output filename/path under results_folder (overrides output_prefix)")
    parser.add_argument(
        "--output-prefix",
        type=str,
        default="",
        help="Prefix for output files: creates {prefix}.png and {prefix}.csv in results_folder",
    )
    ...
    if cfg_path.exists():
        ...
        results_folder = args.results_folder or cfg_defaults.get("results_folder", "")
        ...
        results_folder = args.results_folder or ""
        sep = args.sep or ";"
    ...
    # Resolve results_folder (required for all outputs; everything is saved here)
    res_folder = Path(results_folder).expanduser() if results_folder else (repo_root / "results")
    if not res_folder.is_absolute():
        res_folder = (repo_root / res_folder).resolve()
    else:
        res_folder = res_folder.resolve()

    # All outputs go under results_folder: PNG and CSV
    # --out or config "out" is path relative to results_folder; else use output_prefix
    if out_path:
